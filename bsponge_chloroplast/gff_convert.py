...
def _flatten_features(rec):
    ...
    for f in rec.features:
        cur = [f]
        while len(cur) > 0:
			print cur[0].location
			if cur[0].location.strand == 1:
				nloc = FeatureLocation( min( cur[0].location.start, cur[0].location.end ), max( cur[0].location.start, cur[0].location.end ), 1 ) 
			else: 
				nloc = FeatureLocation( max( cur[0].location.start, cur[0].location.end ), min( cur[0].location.start, cur[0].location.end ), -1 ) 
			#cur[0].location = nloc
			nextf = []
			for curf in cur:
				out.append(curf)
